...
def check_password(password):
    ...
    if t2mod.is_to_short(password):
        print(f"{message} is to short")
        return False
    elif t2mod.is_to_long(password):
        print(f"{message} is too long")
        return False
    elif not t2mod.has_lower(password):
        print(f"{message} {less_than} lowercase letter")
        return False
    elif not t2mod.has_upper(password):
        print(f"{message} {less_than} uppercase letter")
        return False
    elif not t2mod.has_digit(password):
        print(f"{message} {less_than} digit")
        return False
    elif not t2mod.has_special_characters(password):
